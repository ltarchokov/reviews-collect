...
DEFECTS = {
    ...
    # Корни негативные: просто «вкус» ловит «вкусный» в похвале.
    "вкус и приём": ("невкус", "не вкус", "безвкус", "горьк", "кисл", "привкус", "противн", "тошнот", "глотать"),
...
def build(
    ...
    if rated:
        ...
        if decl and base_rated:
            ...
            d.p(
                f"Площадка показывает распределение **{dtot}** оценок на этих "
                f"карточках (чуть меньше заявленного числа отзывов — часть "
                f"отзывов исключена из рейтинга), собрать удалось "
                f"**{len(base_rated)}** с оценкой"
    ...
    if neg:
        neg_text = sum(1 for r in neg if any(len(str(r.get(f) or "")) > 20 for f in ("text", "cons")))
        d.h2(f"Негатив (1–3 звезды): {len(neg)} отзывов, с текстом {neg_text}")
        d.p(
            f"Всё, что ниже про жалобы, опирается на {neg_text} негативных "
            "отзывов с текстом. Остальные — оценка без слов, тему по ним "
            "не определить."
        )
        ...
        # что-то сделать на производстве. Считается только по претензиям
        # к продукту: отзыв про доставку со словами «сам кофе вкусный»
        # иначе попадал в тему «вкус» и шёл в цитаты.
        about = [
            r for r in neg
            if _bucket(" ".join(str(r.get(f) or "") for f in ("text", "cons", "title")))
            == "о самом продукте"
        ]
        defects = _defects(about)
        if defects:
            d.h3("В чём претензия")
            d.table(
                ["Дефект", "Отзывов", "Доля претензий к продукту"],
                [[name, n, _pct(n, len(about))] for name, n in defects[:12]],
                align="lrr",
            )
            d.p(
            ...
            for name, n in defects[:3]:
                ...
                for q in _quotes(about, DEFECTS[name]):
                    d.quote(
        ...
        if len(cards) >= 3:
            ...
            # До 11 карточек показываем все; больше — верх и низ, между
            # ними многоточие. Иначе при трёх карточках каждая печаталась
            # дважды: и в «верхних восьми», и в «нижних трёх».
            shown = cards if len(cards) <= 11 else cards[:8] + [None] + cards[-3:]
            rows = [
                ["…", "", ""] if c is None else [f"{c[0]:.1f}%", f"{c[2]} из {c[1]}", c[3][:52]]
                for c in shown
            ]
